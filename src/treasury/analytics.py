"""
Module d'analytics et transformations de données
...
def convert_generic_to_analytics_format(df_pnl: pd.DataFrame) -> pd.DataFrame:
    """
    Convertit le format PnL générique vers format analytics pour visualisations
    
    Args:
        df_pnl: DataFrame avec résultats PnL génériques
        
    Returns:
        pd.DataFrame: Format analytics standardisé
    """
    try:
        if df_pnl.empty:
        ...
        for _, row in df_pnl.iterrows():
            # Calcul tenor depuis time_to_maturity_years
        ...
        if not df_analytics.empty:
            ...
            # Montants en millions pour visualisations
...
def calculate_portfolio_concentrations(df_analytics: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    """
    Calcule les concentrations du portefeuille par différentes dimensions
    
    Args:
        df_analytics: DataFrame format analytics
        
    Returns:
        Dict[str, pd.DataFrame]: Concentrations par dimension
    """
    concentrations = {}
...
def calculate_time_profile(df_analytics: pd.DataFrame) -> pd.DataFrame:
    """
    Calcule le profil temporel du portefeuille (échéances)
    
    Args:
        df_analytics: DataFrame format analytics
        
    Returns:
        pd.DataFrame: Profil temporel
    """
    try:
        if df_analytics.empty:
            return pd.DataFrame()
...
def calculate_pnl_attribution(df_analytics: pd.DataFrame) -> Dict[str, pd.DataFrame]:
    """
    Attribution du PnL par différentes dimensions
    
    Args:
        df_analytics: DataFrame format analytics
        
    Returns:
        Dict[str, pd.DataFrame]: Attribution PnL par dimension
    """
    attributions = {}
...
def calculate_performance_metrics(df_analytics: pd.DataFrame) -> Dict[str, float]:
    """
    Calcule des métriques de performance du portefeuille
    ...
    try:
        ...
        if 'pnl_total' in df_analytics.columns and len(df_analytics) > 1:
            ...
            # Ratio de Sharpe approximatif (PnL moyen / volatilité)
            if pnl_std > 0:
        ...
        # Deals urgents (échéance <= 7 jours)
...
def export_analytics_to_excel(df_analytics: pd.DataFrame, filename_prefix: str = "analytics") -> bytes:
    """
    Exporte l'analyse vers Excel avec multiple feuilles
    
    Args:
        df_analytics: DataFrame format analytics
        filename_prefix: Préfixe du fichier
...
def validate_analytics_data(df_analytics: pd.DataFrame) -> Dict[str, any]:
    """
    Valide la qualité des données analytics
    
    Args:
        df_analytics: DataFrame à valider
